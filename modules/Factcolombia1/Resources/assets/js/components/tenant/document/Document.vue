            ...
            <template slot="items" slot-scope="props">
              ...
              <td>
                ...
                <div>
                  ...
                  <template
                    v-if="props.item.type_document_id != 1"
                  >({{props.item.reference.prefix}}{{props.item.reference.number}})</template>
                </div>
              </td>
              <td>{{props.item.state_document.name}}</td>
              <td>{{(props.item.acknowledgment_received != null) ? ((props.item.acknowledgment_received == 1) ? 'Aceptado' : 'Rechazado') : ''}}</td>
              <td>{{props.item.currency.code}}</td>
              <td>{{ratePrefix()}}{{props.item.sale | numberFormat}}</td>
              <td>{{ratePrefix()}}{{props.item.total_discount}}</td>
              ...
              <td>
                <v-menu
                  v-model="props.item.acctions"
                  :close-on-content-click="false"
                  :nudge-width="200"
                  offset-x
                >
                  <template v-slot:activator="{on}">
                    <v-icon small v-on="on" color="green darken-2">fas fa-eye</v-icon>
                  </template>
                  <v-card>
                    <v-list>
                      <v-list-tile avatar>
                        ...
                        <v-list-tile-content>
                          <v-list-tile-title>{{props.item.type_document.name}}</v-list-tile-title>
                          <v-list-tile-sub-title>{{props.item.prefix}}{{props.item.number}}</v-list-tile-sub-title>
                          <v-list-tile-sub-title class="caption">{{props.item.cufe}}</v-list-tile-sub-title>
                        </v-list-tile-content>
                        <v-list-tile-action>
                          <v-btn
                            icon
                            :loading="props.item.loading"
                            @click="query(props.item)"
                            :disabled="props.item.state_document_id == 6"
                          >
                    ...
                    <v-list>
                      <v-list-tile>
                        ...
                        <v-list-tile-action>
                          <v-btn icon @click="sendEmail(props.item)">
                            <v-icon small>email</v-icon>
                          </v-btn>
                        </v-list-tile-action>
                      </v-list-tile>
                      <v-list-tile>
                        <v-list-tile-title>XML</v-list-tile-title>
                        <v-list-tile-action>
                          <v-btn
                            icon
                            :href="`${api_download}${props.item.response_api_invoice.urlinvoicexml}`"
                            target="_blank"
                          >
                      ...
                      <v-list-tile>
                        ...
                        <v-list-tile-action>
                          <v-btn
                            icon
                            :href="`${api_download}${props.item.response_api_invoice.urlinvoicepdf}`"
                            target="_blank"
                          >
                            <v-icon small>fas fa-download</v-icon>
                          </v-btn>
                    ...
                    <v-divider></v-divider>
                    <!--<v-list v-if="props.item.log_documents.length > 0">
                                            <v-list-tile>
                                                <v-list-tile-content>
                                                    <v-list-tile-title>Log</v-list-tile-title>
                                                    <v-list-tile-sub-title>{{props.item.log_documents[0].payload}}</v-list-tile-sub-title>
                                                </v-list-tile-content>
                                            </v-list-tile>
                    </v-list>-->
                  </v-card>
...
<!--                            <td>{{ratePrefix()}}{{props.item.price | numberFormat}}</td> -->
                      ...
                      <v-flex v-if="showTotalAndSave" xs12 sm12 md12 lg12 xl12>
                        <v-layout justify-end>
                          <v-btn block @click="dialogItems = !dialogItems">
                        ...
                        <v-layout justify-end>
                          ...
                          >Total venta: {{ratePrefix()}}{{document.sale | numberFormat}}</div>
                        </v-layout>
                        <v-layout justify-end>
                          <div
                            class="text-uppercase title"
                          >Total descuento(-): {{ratePrefix()}}{{document.total_discount | numberFormat}}</div>
                        </v-layout>
                        <template v-for="(tax, index) in document.taxes">
                          <v-layout v-if="((tax.total > 0) && (!tax.is_retention))" justify-end>
                            <div
                              class="text-uppercase title"
                            >{{tax.name}}(+): {{ratePrefix()}}{{Number(tax.total).toFixed(2) | numberFormat}}</div>
                          </v-layout>
                        </template>
                        <v-layout justify-end>
                          <div
                            class="text-uppercase title"
                          >Subtotal: {{ratePrefix()}}{{document.subtotal | numberFormat}}</div>
                        </v-layout>
                        ...
                        <v-layout justify-end>
                          <div
                            class="text-uppercase title"
                          >Total: {{ratePrefix()}}{{document.total | numberFormat}}</div>
                        </v-layout>
                      </v-flex>
                    </v-layout>
                  </v-container>
...
export default {
  ...
  computed: {
    ...
    showTotalAndSave() {
      return true; //((this.document.hasOwnProperty('items')) && (this.document.items.length > 0));
    },
    typeNoteDocuments() {
    ...
    document: {
      handler(val) {
        ...
        val.subtotal = val.items
          ...
          .toFixed(2);
        val.sale = val.items
          .reduce((p, c) => Number(p) + Number(c.price * c.quantity), 0)
          .toFixed(2);
        ...
        this.taxes.forEach(tax => {
          if (tax.is_retention && tax.in_base && tax.apply) {
            ...
            if (Number(totalRetentionBase) >= Number(val.subtotal))
  ...
  methods: {
    ...
    validate(scope) {
      this.$validator.validateAll(scope).then(valid => {
        if (valid) {
          this.loading = true;
          this.generateNoteService();

//          console.log(JSON.stringify(this.noteService));
//          return false;

          this.document.note_service = this.noteService;

          axios
            .post(`${this.route}/note`, this.document)
            .then(response => {
              this.$setLaravelMessage(response.data);
    ...
    query(document) {
      ...
      axios
        .put(`${this.route}/query/${document.id}`)
        .then(response => {
          this.$setLaravelMessage(response.data);
    ...
    editDocument(document) {
      ...
      //asigno data a la nota para api service
      this.noteService.customer = {
        identification_number: documentClone.client.identification_number,
        name: documentClone.client.name,
        phone: documentClone.client.phone,
        address: documentClone.client.address,
        email: documentClone.client.email,
        merchant_registration: "0000-00",
        type_document_identification_id:
        documentClone.client.type_identity_document_id,
        type_organization_id: documentClone.client.type_person_id,
        municipality_id: 149,
        type_regime_id: 2
      };
      ...
      documentClone = {
        ...
        total: 0,
        sale: 0,
        payment_form_id: documentClone.payment_form_id,
        payment_method_id: documentClone.payment_method_id,
        correlative_api: documentClone.correlative_api,
        response_api_cufe:  documentClone.response_api_cufe
      };
    ...
    sendEmail(document) {
      ...
      axios
        .post(`${this.route}/sendEmail/${document.correlative_api}/${document.client_id}`)
        .then(response => {
          this.$setLaravelMessage(response.data);
        })
        .catch(error => {
      ...
      this.document.items.forEach(element => {
        ...
        if(find)
        {
          ...
          tax.push({
            ...
            taxable_amount: this.cadenaDecimales((Number(element.price) * Number(element.quantity)) - element.discount)
          });
        }
      });
    ...
    {
      let id = this.document.type_document_id
      let id_service = 0
    ...
    generateNoteService() {
      ...
      this.noteService.type_document_id = this.getTypeDocumentService(),
      this.noteService.date = "";
      this.noteService.time = "";

      this.noteService.billing_reference = {
        number: String(contex.document.correlative_api),
        uuid: contex.document.response_api_cufe,
        issue_date: contex.document.date_issue
      };

      this.noteService.tax_totals = this.getTaxTotal();
      this.noteService.with_holding_tax_total = this.getWithHolding();
      if(this.noteService.type_document_id == 4)
      {
        this.noteService.legal_monetary_totals = this.getLegacyMonetaryTotal();
        this.noteService.credit_note_lines = this.getCreditnoteLines();
      }
      else if(this.noteService.type_document_id == 5)
      {
        this.noteService.requested_monetary_totals = this.getLegacyMonetaryTotal();
        this.noteService.debit_note_lines = this.getCreditnoteLines();
      }
    }
  }
};
